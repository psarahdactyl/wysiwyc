...
int in_on_out(const Segment& seg, const Segment_set& intersecting_segs, const Point& intersection, const Segment_set& shape)
{
	...
	if (CGAL::do_overlap(bounds, centroid.bbox()))
	{
		...
		Segment intersecting_seg = intersecting_segs[0];
		//std::cout << "intersecting segment " << intersecting_seg << std::endl;
		K::Construct_projected_point_2 project;
		Point proj = project(intersecting_seg.supporting_line(), centroid);
		...
		bool equal_signs = (CGAL::sign(tri_area) == CGAL::sign(poly_area));
		...
		if (intersection != proj)
		{
			...
			equal_signs = (CGAL::sign(tri_area) == CGAL::sign(poly_area));
			...
			if (int_proj.is_horizontal())
			{
				...
				CGAL::Lazy_exact_nt<CGAL::Gmpq> seg_slope = intersecting_seg.direction().dx();
			...
			else if (int_proj.is_vertical())
			{
				...
				CGAL::Lazy_exact_nt<CGAL::Gmpq> seg_slope = intersecting_seg.direction().dy();
			...
			else
			{
				...
				CGAL::Lazy_exact_nt<CGAL::Gmpq> seg_y =	intersecting_seg.direction().dy();
				CGAL::Lazy_exact_nt<CGAL::Gmpq> int_proj_x = int_proj.direction().dx();
				CGAL::Lazy_exact_nt<CGAL::Gmpq> seg_x =	intersecting_seg.direction().dx();
				if (!(CGAL::sign(int_proj_y) == CGAL::sign(seg_y)) 
					&& !(CGAL::sign(int_proj_x) == CGAL::sign(seg_x)))
				{
					std::cout << "triangle area " << tri_area << std::endl;
					std::cout << "polygon area " << poly_area << std::endl;
					std::cout << "triangle area sign " << CGAL::sign(tri_area) << std::endl;
					std::cout << "polygon area sign " << CGAL::sign(poly_area) << std::endl;
					tri_area *= -1;
					equal_signs = false;
		...
		else
		{
			...
			// find if centroid is inside shape
			Point_set triangle_points = { intersection, intersecting_seg.target(), centroid };
			Polygon centroid_triangle(triangle_points.begin(), triangle_points.end());
			tri_area = centroid_triangle.area();
			poly_area = shape_polygon.area();
		}
		...
		else if (intersecting_seg.has_on(centroid))
		{
			// on
			return 1;
		}
